...
found:
  ...
  if ((p->trapcopy = (struct trapframe *)kalloc()) == 0)
  {
    freeproc(p);
    release(&p->lock);
    return 0;
    ...
    for (p = proc; p < &proc[NPROC]; p++)
    {
      ...
      if (p->state == RUNNABLE)
      {
        ...
        for(struct proc * i=proc;i<&proc[NPROC];i++){
          if(i->ctime<p->ctime && i->state==RUNNABLE){
            flag=1;
            break;
          }
        }
...
void procdump(void)
{
  ...
  for (p = proc; p < &proc[NPROC]; p++)
  {
    ...
    if (p->state >= 0 && p->state < NELEM(states) && states[p->state])
      ...
      state = "???";
    printf("%d %s %s %d %d", p->pid, state, p->name,p->priority,ticks);
    printf("\n");
  }
}
...
void update_time()
{
  ...
  {
  if(p->pid>=9 &&p->pid<=14 ){
  printf("%d %d %d %d\n",p->pid,p->priority,ticks,p->lastscheduledticks);}
    acquire(&p->lock);
    if (p->state == RUNNING)
    {
      p->rtime++;
